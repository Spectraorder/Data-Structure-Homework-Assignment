...
class Matrix :
    ...
    def max_value(self):
        maxOut = self._theGrid[0,0]
        for i in range(0, self.numRows()):
            for k in range(0, self.numCols()):
                if(self._theGrid[i,k]>maxOut):
                    maxOut = self._theGrid[i,k]
        return maxOut
    ...
    def transpose( self ):
        outMat = Matrix(self.numCols(), self.numRows())
        for row in range(0, self.numRows()):
            for col in range(0, self.numCols()):
                outMat._theGrid[col, row] = self._theGrid[row, col]
        return outMat

    # Creates and returns a new matrix that results from matrix addition.
    def __add__( self, rhsMatrix ):
        outMat = Matrix(self.numRows(), self.numCols())
        for row in range(self.numRows()):
            for col in range(self.numCols()):
                outMat._theGrid[row, col] = self._theGrid[row, col] + rhsMatrix._theGrid[row, col]
        return outMat
    ...
    def __mul__( self, rhsMatrix ):
        outMat = Matrix(self.numRows(), rhsMatrix.numCols())
        outMat._theGrid.clear(0)
        for row in range(0, outMat.numRows()):
            for col in range(0, outMat.numCols()):
                for i in range(0, self.numCols()):
                    outMat._theGrid[row, col] += self._theGrid[row, i] * rhsMatrix._theGrid[i, col]
        return outMat
